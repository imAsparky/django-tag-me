...
def _get_manifest_path():
    ...
    custom_path = getattr(settings, "TAGME_CSS_MANIFEST_PATH", None)
...
def get_tag_me_css():
    ...
    1. Your main project's manifest (if TAGME_CSS_MANIFEST_PATH is set)
    ...
    if not manifest:
        ...
        if getattr(settings, "TAGME_CSS_MANIFEST_PATH", None):
            fallback = getattr(settings, "TAGME_CSS_FALLBACK", "css/styles.css")
    ...
    css_key = getattr(settings, "TAGME_CSS_MANIFEST_KEY", "style.css")
    ...
    css_base_path = getattr(settings, "TAGME_CSS_BASE_PATH", "tag_me/dist")
...
if settings.DEBUG:
    ...
    def debug_manifest():
        ...
        try:
            ...
            print(
                f"CSS Manifest Key: {getattr(settings, 'TAGME_CSS_MANIFEST_KEY', 'style.css')}"
            )
            print(
                f"CSS Base Path: {getattr(settings, 'TAGME_CSS_BASE_PATH', 'tag_me/dist')}"
            )
